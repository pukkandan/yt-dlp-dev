...
class CurlCFFIRH(RequestHandler, InstanceStoreMixin, ImpersonateHandlerMixin):
    ...
    _SUPPORTED_IMPERSONATE_TARGETS = requests.BrowserType._member_names_
    ...
    def _send(self, request: Request):
        ...
        # FIXME
        old = session.curl.setopt
        session.curl.setopt = lambda k, v: old(k, v.decode() if isinstance(v, bytes) else v)
        ...
        try:
            ...
            if error_code in (CurlECode.PEER_FAILED_VERIFICATION, CurlECode.OBSOLETE51):
                ...
                curl_response = requests.cookies.Response(
                    curl=session.curl,
                    request=requests.cookies.Request(
                        url=request.url,
                        headers=requests.headers.Headers(request.headers),
                        method=request.method,
                    ))
