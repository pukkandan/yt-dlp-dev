...
class Updater:
    ...
    def _new_version_info(self):
        if not self._version_cache:
            ...
            self._version_cache = json.loads(self.ydl.urlopen(API_URL).read().decode())
    ...
    def get_release_hash(self):
        """Get the hash of the latest release"""
    ...
    def _report_network_error(self, action, delim=';'):
        self._report_unable(f'{action}{delim} Visit  https://github.com/{REPOSITORY}/releases/latest', True)
